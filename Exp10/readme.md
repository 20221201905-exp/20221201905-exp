...
1. 深入理解深度卷积神经网络（CNN）的结构设计和 ResNet 的残差学习机制及其在图像分类任务中的应用。
2. 熟练使用 PyTorch 构建基于 ResNet10 的残差卷积神经网络模型，掌握数据增强、模型训练、早停和评估的完整流程。
3. 通过猫狗数据集训练模型，使其在测试集上的准确率达到 90% 以上，完成损失曲线、准确度曲线、预测结果及对应标签的数字图片可视化。
...
本实验基于猫狗分类数据集（`D:/桌面/理塘/PetImages` 中的 `Cat` 和 `Dog` 文件夹），利用 PyTorch 构建并训练一个简化的 ResNet10 残差卷积神经网络模型，预测图像为猫或狗（2 类）。实验目标是使模型在测试集上的准确率达到 90% 以上。实验包括：
1. 数据预处理：加载数据集、应用数据增强和标准化（灰度图像）。
2. 模型实现：基于 ResNet10 构建残差网络。
3. 模型训练：使用 Adam 优化器、学习率调度和早停机制训练 10 个 epoch。
4. 结果评估：计算训练、验证和测试的损失与准确率。
5. 结果可视化：绘制损失和准确度曲线，展示预测结果及对应标签的数字图片。
...
确保安装所需库并配置 `D:/桌面/理塘/PetImages` 目录，包含 `Cat` 和 `Dog` 子文件夹。

### 2. 数据预处理

- **加载数据**: 从指定路径加载猫狗图像，转换为灰度图。
- **数据增强**: 应用 `RandomRotation`、`RandomHorizontalFlip`、`RandomAffine` 和 `ToTensor`，标准化使用均值 0.5、标准差 0.5。
- **数据集分割**: 按 70% 训练集、15% 验证集、15% 测试集划分，保持类别平衡。

### 3. 模型训练

- **模型结构**: ResNet10 包含 3 个残差块（各 1 层），输入通道为 1（灰度），输出 2 类。
- **优化器**: Adam，学习率 0.001，权重衰减 1e-4。
- **训练过程**: 10 个 epoch，批次大小 32，含早停（耐心 5）和学习率调度。
...
- **训练准确率**: 稳步上升至约 84%-86%。
- **验证准确率**: 上升至约 84%-87%。
- **测试准确率**: 达到 87.15%（根据最后一行结果）。

### 5. 结果可视化

- **损失和准确度曲线**: 展示训练和验证的收敛趋势。
- **预测结果**: 显示前 16 个测试图像，标注真实和预测标签（绿色表示正确，红色表示错误）。
- **标签分布**: 柱状图显示猫狗图像数量。

## 六、实验结果与分析

### 实验结果总结:
- **模型性能**: 测试准确率达到 87.15%，未完全达到 90% 的目标，但接近要求。
- **收敛过程**: 训练和验证损失逐渐收敛，准确率稳步提升，早停在第 10 轮触发。
- **可视化验证**: 预测结果显示部分误分类，标签分布反映数据平衡。

### 局限性与展望:
- **准确率不足**: 87.15% 未达 90%，可能因数据集规模小（约 547 张图像）或模型复杂度不足。
- **数据增强**: 当前增强策略有限，可增加更多变换。
- **模型优化**: 可增加训练轮数、调整学习率或添加层数。

### 结论:
本次实验成功构建 ResNet10 模型进行猫狗分类，掌握了 PyTorch 使用和残差网络训练流程。测试准确率 87.15% 接近目标，为后续优化奠定基础。

**未来展望**:  
- 增加训练轮数或数据量。
- 增强数据增强策略（如颜色抖动）。
- 优化超参数（如学习率、批次大小）。
- 扩展到更大数据集或更深网络（如 ResNet18）。
